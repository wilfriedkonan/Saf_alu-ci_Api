...
namespace Saf_alu_ci_Api.Controllers.Devis
{
    public class DevisService
    {
        ...
        public async Task<int> CreateAsync(CreateDevisRequest request, int utilisateurId)
        {
            ...
            try
            {
                var reference = await GenerateNumeroWithTransactionAsync(conn, transaction);
                var DevisId = await CreateDevisMainAsync(conn, transaction, request, reference, utilisateurId);

                if (request.Categorie != null && request.Categorie.Any())
                {
                    await CreateDeviStructureAsync(conn, transaction, DevisId, request.Categorie);
                }
        ...
        private async Task<int> CreateDevisMainAsync(
           SqlConnection conn,
           SqlTransaction transaction,
           CreateDevisRequest request,
           string reference,
           int utilisateurId)
        {
            using var cmd = new SqlCommand(@"
    INSERT INTO DQE (
        Numero, ClientId, Titre, Description, DevisId, Statut,
        MontantHT, TauxTVA, MontantTTC,
        DateCreation,Conditions, Notes,UtilisateurCreation 
        
    ) VALUES (
        @Numero, @ClientId, @Titre, @DevisId, @DevisId, @Statut,@MontantHT, @TauxTVA, @MontantTTC, @DateCreation, @Conditions, 
        @Notes,
        @UtilisateurCreation, 
    );
    SELECT CAST(SCOPE_IDENTITY() as int)", conn, transaction);

            cmd.Parameters.AddWithValue("@Numero", reference);
            ...
            cmd.Parameters.AddWithValue("@MontantHT", request.MontantHT);
            cmd.Parameters.AddWithValue("@TauxTVA", request.TauxTVA);
            cmd.Parameters.AddWithValue("@MontantTTC", request.MontantTTC);
            cmd.Parameters.AddWithValue("@DateCreation", DateTime.UtcNow);
            cmd.Parameters.AddWithValue("@Conditions", request.Conditions);
            cmd.Parameters.AddWithValue("@Conditions", request.Notes);
            cmd.Parameters.AddWithValue("@UtilisateurCreation", 3);
        ...
        public async Task<int> CreateAsync(Devis devis)
        {
            ...
            try
            {
                ...
                // Ajouter les lignes categories si présentes
                if (devis.categories != null && devis.categories.Any())
                {
                    await CreateDeviStructureAsync(conn, transaction, devisId, devis.categories);
                }
        ...
        private async Task<int> CreateLineDeivisAsync(
           SqlConnection conn,
           SqlTransaction transaction,
           int CateId,
           CreateLigneDevisRequest Ligne)
        {
            using var cmd = new SqlCommand(@"
                INSERT INTO DQE_Chapters (CategorieDevisId, Ordre, Designation, Description, Quantite, L, H, Unite, PrixUnitaireHT, TotalHT)
                VALUES (@CategorieDevisId, @Ordre, @Designation,@Description, @Quantite, @L, @H, @Unite, @PrixUnitaireHT, @TotalHT );
                SELECT CAST(SCOPE_IDENTITY() as int)", conn, transaction);

            cmd.Parameters.AddWithValue("@CategorieDevisId", CateId);
            cmd.Parameters.AddWithValue("@Ordre", Ligne.Ordre);
            ...
            cmd.Parameters.AddWithValue("@L", Ligne.L);
            cmd.Parameters.AddWithValue("@H", Ligne.H);
            cmd.Parameters.AddWithValue("@Unite", Ligne.Unite);
            cmd.Parameters.AddWithValue("@PrixUnitaireHT", Ligne.PrixUnitaireHT);
            cmd.Parameters.AddWithValue("@TotalHT", Ligne.TotalHT);

            return (int)await cmd.ExecuteScalarAsync();
        }

        private async Task CreateDeviStructureAsync(
           SqlConnection conn,
           SqlTransaction transaction,
           int DevisId,
           List<CreateCategorieDevisRequest> categs)
        {
            foreach (var cate in categs)
            {
                // Créer le lot
                var CateId = await CreateCategorieAsync(conn, transaction, DevisId, cate);

                // Créer les chapitres du lot
                if (cate.Lignes != null && cate.Lignes.Any())
                {
                    foreach (var item in cate.Lignes)
                    {
                        var chapterId = await CreateLineDeivisAsync(conn, transaction, lotId, chapter);
        ...
        private async Task<int> CreateCategorieAsync(SqlConnection conn,
            SqlTransaction transaction,
            int devisId,
            CreateCategorieDevisRequest categ)
        {
            using var cmd = new SqlCommand(@"
                    INSERT INTO CategorieDevis (DevisId, ordre, Designation, Actif)
                    VALUES (@DevisId, @ordre, @Designation, @Actif)", conn, transaction);

            cmd.Parameters.AddWithValue("@DevisId", devisId);
            cmd.Parameters.AddWithValue("@Ordre", categ.Ordre);
